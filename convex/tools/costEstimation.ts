...
export const estimateRelocationCosts = internalMutation({
  ...
  handler: async (ctx, args): Promise<string> => {
    const { destinationCity, includeFlight, includeHousing, includeMoving, familySize } = args;

    // Call the OpenAI action for intelligent cost estimation
    const costAnalysis: string = await ctx.scheduler.runAfter(0, internal.tools.costEstimation.analyzeWithOpenAI, {
      destinationCity,
      includeFlight,
      includeHousing,
      includeMoving,
  ...
  handler: async (ctx, args): Promise<string> => {
    ...
    if (exaApiKey) {
      try {
        ...
        searchResults = search.results.map(r => `${r.title}: ${r.text?.substring(0, 500)}`).join('\n\n');
      } catch (error) {
        console.error("Exa search error:", error);
      }
    }

    // Base cost data structure for fallback
    const baseCosts = {
      flight: {
        ...
        messages: [
          {
            ...
            Consider:
            1. Current market rates for the destination city
            2. Family size impact on costs
            3. Seasonal variations
            4. Hidden costs often overlooked
            5. Currency conversions if applicable
    ...
    if (includeFlight) {
      ...
      breakdown.push(`✈️ Flights: £${baseCosts.flight.economy} - £${baseCosts.flight.business}`);
    }

    if (includeHousing) {
      const housingCost = familySize === 1 ? baseCosts.housing.one_bed : 
                         familySize === 2 ? baseCosts.housing.two_bed : 
                         baseCosts.housing.family;
      totalMin += housingCost;
      totalMax += housingCost * 1.5;
      breakdown.push(`🏠 First month housing: £${housingCost} - £${Math.round(housingCost * 1.5)}`);
    ...
    if (includeMoving) {
      ...
      breakdown.push(`📦 Moving services: £${baseCosts.moving.minimal} - £${baseCosts.moving.premium}`);
    }

    // Always include setup costs
    const setupTotal = Object.values(baseCosts.setup).reduce((a, b) => a + b, 0);
    totalMin += setupTotal;
    totalMax += setupTotal;
    breakdown.push(`🔧 Setup costs (utilities, groceries, etc.): £${setupTotal}`);
