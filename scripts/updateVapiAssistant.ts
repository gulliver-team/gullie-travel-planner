...
const systemPrompt = readFileSync(join(process.cwd(), 'public/SYSTEM.md'), 'utf-8');
...
const tools = [
  {
    ...
    function: {
      ...
      description: "Search for relocation options when user provides origin and destination cities",
      parameters: {
        type: "object",
        properties: {
          origin_city: {
    ...
    messages: [
      {
        type: "request-start",
        content: "Let me search for the best relocation options from {{origin_city}} to {{destination_city}}. This will take just a moment..."
      },
      {
      ...
      {
        type: "request-failed",
        content: "I'm having trouble searching for relocation options right now. Let me try another approach."
      }
    ]
  },
  {
  ...
  },
  {
    ...
    function: {
      ...
      description: "Confirm whether user wants detailed explanation or PDF report",
      parameters: {
        type: "object",
        properties: {
          send_pdf: {
            type: "boolean",
            description: "Whether user wants PDF report instead of voice explanation"
          }
        },
        required: ["send_pdf"]
      }
...
async function updateAssistant() {
  try {
    ...
    const response = await fetch(`https://api.vapi.ai/assistant/${VAPI_ASSISTANT_ID}`, {
      ...
      body: JSON.stringify({
        name: "Maya - Global Mobility Expert",
        firstMessage: "Hi there, I'm Maya, your global mobility expert. I'm here to help you navigate your international relocation. Could you tell me which country and city you're moving from, and where you're planning to relocate to?",
        model: {
          provider: "openai",
          model: "gpt-4o",
          messages: [
        ...
        serverMessages: ["transcript", "tool-calls", "end-of-call-report", "status-update"],
